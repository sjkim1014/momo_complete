...
$(document).ready(function() {
	...
	if ($("#replyupdate").attr('name') != reader_no) {
		$("#replyupdate").hide();
	}
	if ($("#replydelete").attr('name') != reader_no) {
		$("#replydelete").hide();
	}
	if ($("#replyupdate_2").attr('name') != reader_no) {
		$("#replyupdate_2").hide();
	}
	if ($("#replydelete_2").attr('name') != reader_no) {
		$("#replydelete_2").hide();
	}
...
<table class="table table-striped">
		...
		%>
			<tr class="replyrow" name=<%=list.get(i).getBc_no() %> >
				...
				<% 
					if (list.get(i).getU_no() == (int) session.getAttribute("u_no")) {
				%>
				<td><button type="button" id="replyupdate" name=<%=list.get(i).getU_no() %> class="btn btn-warning-outline btn-sm">수정 </button></td>
				<td><button type="button" id="replydelete" name=<%=list.get(i).getU_no() %> class="btn btn-warning-outline btn-sm">삭제</button></td>
				<% 
					} else {
				%> 
				<td></td>
		...
		%>
			<tr class="replyrow" name=<%=list.get(j).getBc_no() %> >
				...
				<% 
					if (list.get(j).getU_no() == (int) session.getAttribute("u_no")) {
				%>
				<td> </td>
				<td><button type="button" id="replyupdate_2" name=<%=list.get(j).getU_no() %> class="btn btn-warning-outline btn-sm">수정</button></td>
				<td><button type="button" id="replydelete_2" name=<%=list.get(j).getU_no() %> class="btn btn-warning-outline btn-sm">삭제</button></td>
				<% 
					} else {
				%> 
				<td></td>
