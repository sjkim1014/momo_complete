...
@WebServlet("/banners/file")
public class BannersFileUploadController extends HttpServlet {
	
	private static final long serialVersionUID = 1L;
	
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.getRequestDispatcher("/view/admin/banners/upload.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		resp.getWriter().print("<html><head><meta charset=\"UTF-8\"></head><script type='text/javascript'>"
				+ "window.onload = function() {"
				+ "opener.sendData("+file.getFile_no()+",'"+file.getFile_originname()+"');  window.close();}</script></html>");
