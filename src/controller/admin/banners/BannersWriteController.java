...
@WebServlet("/banners/write")
public class BannersWriteController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (req.getSession().getAttribute("login") == null) {
		  ...
		  //게시글 조회수행
		  Banners bannerview = banService.view(banners);
				
		  //MODEL 전달
		  req.setAttribute("bannerview", bannerview);
		  
//		  req.getSession().setAttribute("bn_no", banners.getBn_no());
		
		
		
		req.getRequestDispatcher("/view/admin/banners/write.jsp").forward(req, resp);
